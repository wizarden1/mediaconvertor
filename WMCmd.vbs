...
' FileName:            WMCMD.VBS
'
' Abstract:            Windows Media Encoder Command Line Script Utility
'                      Use Cscript.exe wmcmd.vbs /? for Help
'
' Last Update:         October 11, 2007
'
' Visit http://www.citizeninsomniac.com/WMV for latest (unofficial) updates.
'*****************************************************************************
...
if instr( LCase(WScript.Fullname),"cscript.exe" ) = 0 then
	...
	WshShell.Popup "This script must be run using cscript.exe from a command window." , 0 , "Windows Media Encoder Command Line Script Utility" , 64
	WScript.Quit()
end if
...
dim g_strProductName
dim g_strProductCopyright
dim g_strInput
dim g_strAudioInput
dim g_strOutput
...
dim g_strVideoEncoderRegistryKey
dim g_strLogFile
...
dim g_intDuration
dim g_intVerbose
dim g_intAudioSourceDuration
dim g_intVideoSourceDuration
...
dim g_intVideoCompOpt
dim g_intVideoDenoise
dim g_intVideoDquantOption
dim g_intVideoDquantStrength
dim g_intVideoBDeltaQP
dim g_intVideoLoopfilter
dim g_intVideoMedian
dim g_intVideoEdgeRemoval
dim g_intVideoNumThreads
dim g_intVideoRangeRedux
dim g_intVideoOverlap
dim g_intVideoScaling
dim g_intVideoLookahead
dim g_intVideoLookaheadRC
dim g_intVideoMBModeCost
dim g_intVideoMMatch
dim g_intVideoMSLevel
dim g_intVideoMSRange
dim g_intVideoMVCoding
dim g_intVideoMVCost
dim g_intVideoBFrameDist
dim g_intVideoPercOpt
dim g_intVideoType
dim g_intVideoCodedWidth
dim g_intVideoCodedHeight
dim g_strVideoPreset
dim g_intVideoThreadAffinity

dim g_EncoderRegSettings(30, 1)
...
g_strProductName = "Microsoft (R) Windows Media Encoder Command Line Script Utility"
g_strProductCopyright = "Copyright (C) Microsoft Corporation. All rights reserved."
g_strInput = ""
g_strAudioInput = ""
g_strOutput = ""
g_strOutputString = ""
...
g_strProfileLoad = ""
g_strProfile = ""
g_strVideoEncoderRegistryKey = "HKCU\Software\Microsoft\Scrunch\WMVideo"
...
g_intDuration = -1
g_intVerbose = 2
g_intAudioSourceDuration = -1
g_intVideoSourceDuration = -1
...
g_blnDurationError = false
g_blnLogOutput = false
...
g_intVideoCompOpt = -128
g_intVideoDenoise = -128
g_intVideoDquantOption = -128
g_intVideoDquantStrength = -128
g_intVideoBDeltaQP = -128
g_intVideoLoopfilter = -128
g_intVideoMedian = -128
g_intVideoEdgeRemoval = -128
g_intVideoNumThreads = -128
g_intVideoRangeRedux = -128
g_intVideoOverlap = -128
g_intVideoScaling = -128
g_intVideoLookahead = -128
g_intVideoLookaheadRC = -128
g_intVideoMBModeCost = -128
g_intVideoMMatch = -128
g_intVideoMSLevel = -128
g_intVideoMSRange = -128
g_intVideoMVCoding = -128
g_intVideoMVCost = -128
g_intVideoBFrameDist = -128
g_intVideoPercOpt = -128
g_intVideoType = -128
g_intVideoCodedWidth = -128
g_intVideoCodedHeight = -128
g_strVideoPreset = ""
g_intVideoThreadAffinity = -128

for i = 0 to 30 
	g_EncoderRegSettings(i,0) = ""
	g_EncoderRegSettings(i,1) = -128
next
...
if not isObject(g_objEncoder) then
	OutputInfo "Error: Cannot initialize Windows Media Encoder object. Please verify that"
	OutputInfo "Windows Media Encoder 9 Series is correctly installed on your system."
	OutputInfo "If running 64-bit Windows, make sure that the version of cscript.exe used"
	OutputInfo "corresponds to the version (x86/x64) of WME9 installed on your system."
	WScript.Quit()
end if
On Error Goto 0
...
if ParseParameters( colStrArgs ) then
	...
	OutputInfo "Command-line parsing completed."
end if


if not IsNull(g_objLogFile) then
	g_objLogFile.Close
end if

' Some input sources are known to cause the script engine to hang
' Forcefully terminate this script
TerminateEncoderProcess()
...
function OutputInfo( strInfo )
	...
	if g_blnLogOutput then
		g_objLogFile.WriteLine( strInfo )
	end if
end function
...
function IsEncoderRegistryDirty()
	...
	regDirtyFlag = g_objWshShell.RegRead(g_strVideoEncoderRegistryKey & "\WMCmdDirtyFlag")
	
	if err <> 0 then
		bDirtyFlag = false
	else
		if regDirtyFlag = 1 then
			bDirtyFlag = true
		else
			bDirtyFlag = false
		end if
	...
	IsEncoderRegistryDirty = bDirtyFlag

end function

function SaveEncoderRegistrySettings()

	On Error Resume Next
	
	g_EncoderRegSettings(0,0) =  "\Compression Optimization Type"
	g_EncoderRegSettings(1,0) =  "\DenoiseOption"
	g_EncoderRegSettings(2,0) =  "\Dquant Option"
	g_EncoderRegSettings(3,0) =  "\Dquant Strength"
	g_EncoderRegSettings(4,0) =  "\Force B Frame Delta QP"
	g_EncoderRegSettings(5,0) =  "\Force Encoding Width"
	g_EncoderRegSettings(6,0) =  "\Force Encoding Height"
	g_EncoderRegSettings(7,0) =  "\Force LoopFilter"
	g_EncoderRegSettings(8,0) =  "\Force Median"
	g_EncoderRegSettings(9,0) =  "\Force NoiseEdgeRemoval"
	g_EncoderRegSettings(10,0) =  "\Force NumThreads"
	g_EncoderRegSettings(11,0) =  "\Force Range Reduction"
	g_EncoderRegSettings(12,0) =  "\Force Overlap"
	g_EncoderRegSettings(13,0) =  "\Force Video Scaling"
	g_EncoderRegSettings(14,0) =  "\Lookahead"
	g_EncoderRegSettings(15,0) =  "\Lookahead Ratecontrol"
	g_EncoderRegSettings(16,0) =  "\Macroblock Mode Cost Method"
	g_EncoderRegSettings(17,0) =  "\Motion Match Method"
	g_EncoderRegSettings(18,0) =  "\Motion Search Level"
	g_EncoderRegSettings(19,0) =  "\Motion Search Range"
	g_EncoderRegSettings(20,0) =  "\Motion Vector Coding Method"
	g_EncoderRegSettings(21,0) =  "\Motion Vector Cost Method"
	g_EncoderRegSettings(22,0) =  "\NumBFrames"
	g_EncoderRegSettings(23,0) =  "\Perceptual Option"
	g_EncoderRegSettings(24,0) =  "\VideoType"
	g_EncoderRegSettings(25,0) =  "\Encoder Thread Affinity"
	
	for i = 0 to 25
		g_EncoderRegSettings(i,1) = g_objWshShell.RegRead(g_strVideoEncoderRegistryKey & g_EncoderRegSettings(i,0))
	next
...
function RestoreEncoderRegistrySettings()

	On Error Resume Next
	
	for i = 0 to 30
		if (g_EncoderRegSettings(i,0) <> "") and (g_EncoderRegSettings(i,1) <> -128) then
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & g_EncoderRegSettings(i,0), g_EncoderRegSettings(i,1), "REG_DWORD"
		end if
	next
...
' Deletes WVC1 encoder parameters from the registry
function ClearEncoderRegistrySettings()

	On Error Resume Next
		
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Compression Optimization Type"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\DenoiseOption"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Dquant Option"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Dquant Strength"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force B Frame Delta QP"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Encoding Width"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Encoding Height"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force LoopFilter"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Median"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force NoiseEdgeRemoval"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force NumThreads"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Range Reduction"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Overlap"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Force Video Scaling"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Lookahead"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Lookahead Ratecontrol"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Macroblock Mode Cost Method"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Motion Match Method"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Motion Search Level"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Motion Search Range"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Motion Vector Coding Method"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Motion Vector Cost Method"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\NumBFrames"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Perceptual Option"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\VideoType"
	g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\Encoder Thread Affinity"
...
function ParseParameters( args )
	...
	dim colMatches
	dim strMarkOutTime
	dim strArguments, strArgumentsQuoted
	dim strSaveToCfgFile
	dim strLoadFromCfgFile
	dim strCfgFileArguments
	dim colStrConfigArgs
	dim objTextFile
	dim intVideoRangeReduxYR, intVideoRangeReduxYE, intVideoRangeReduxCR, intVideoRangeReduxCE
	dim intArgIndex
	
	' First traverse of the arguments just checks for config saving and loading args
	for intArgIndex=0 to UBound(args)
			
		if args(intArgIndex) = "-s_config" then
			intArgIndex = intArgIndex + 1
		else
			if InStr(args(intArgIndex)," ") > 0 then
				strArgumentsQuoted = strArgumentsQuoted & Chr(34) & args(intArgIndex) & Chr(34) & " "
			else
				strArgumentsQuoted = strArgumentsQuoted & args(intArgIndex) & " "
			end if
			strArguments = strArguments & args(intArgIndex) & " "
		end if

		if args(intArgIndex) = "-config" then
			strLoadFromCfgFile = ReadString( args, intArgIndex, intArgIndex+1 )
			...
			' Splits the argument string into an array using space as separator but respecting quotes
			set reParser = New RegExp
			reParser.Pattern = "\s*(" & Chr(34) & "([^" & Chr(34) & "]*)" & Chr(34) & "|([^" & " " & "]+))\s*"
			reParser.Global = true
			set colMatches = reParser.Execute(strCfgFileArguments)
			ReDim colStrConfigArgs(colMatches.Count - 1)
			for j=0 to colMatches.Count - 1
				colStrConfigArgs(j) = Replace(Trim(colMatches(j).Value), Chr(34), "")
			next			
			set reParser = nothing
	...
	if UBound(args) = 1 then
		select case LCase( args(0) ) 
			...
			OutputInfo "Log file can only be used with a valid encoding session."
			ParseParameters = false
			exit function
	...
	for i=0 to UBound(args)
		...
		select case LCase( args(i) )
			...
			case "-a_input"						
				' Get alternate audio input source file name.
				g_strAudioInput = ReadString( args, i, i+1 )
				if g_strAudioInput = ""  then
					ParseParameters = false
					exit function
				end if
			...
			case "-a_codec"
				...
				if g_strAudioCodec = "WMA9STD" then g_strAudioCodec = "WMASTD"
				if g_strAudioCodec = "WMA9PRO" then g_strAudioCodec = "WMAPRO"
				if g_strAudioCodec = "WMA9LSL" then g_strAudioCodec = "WMALSL"
				if g_strAudioCodec = "WMSP9" then g_strAudioCodec = "WMSP9"

				if g_strAudioCodec <> WMASTD and g_strAudioCodec <> WMAPRO and g_strAudioCodec <> WMALSL and g_strAudioCodec <> WMSPEECH and g_strAudioCodec <> PCM then
			...
			case "-v_codec"
				...
				if g_strVideoCodec <> WMV7 and g_strVideoCodec <> WMV8 and g_strVideoCodec <> WMV9 and g_strVideoCodec <> WVC1 and g_strVideoCodec <> WMS9 and g_strVideoCodec <> UNCOMP and g_strVideoCodec <> MP41 then
			...
			case "-v_quality"
				'Get video crispness (CBR) or quality (VBR)
			...
			case "-v_qp"
				'Get video quantizer parameter (VBR)
				g_intVideoQP = ReadFloat( args, i, i+1 )
				if g_intVideoQP = -1 then
					ParseParameters = false
					exit function
				end if
				g_intVideoQP = Round(g_intVideoQP*2)/2
				if g_intVideoQP < 1.0 or g_intVideoQP > 31.0 then
					OutputInfo "Invalid range for parameter " & args(i)
					ParseParameters = false
					exit function
				end if
			...
			case "-v_complexity"
				'Get video complexity setting on codec 0-5 scale, but give v_performance priority
				if (g_intVideoPerformance = -1) then 
					g_intVideoPerformance = ReadInteger( args, i, i+1 ) * 20
			...
			case "-v_performance"
				'Get video complexity setting on WME9 0-100 scale
			...
			case "-v_profile"
				...
				if g_strVideoDevConf = "SIMPLE" then g_strVideoDevConf = "SP"
				if g_strVideoDevConf = "MAIN" then g_strVideoDevConf = "MP"
				if g_strVideoDevConf = "ADVANCED" then g_strVideoDevConf = "AP"
				' Translate old pre-VC1 Complex Profile to Advanced Profile
				if g_strVideoDevConf = "CP" then g_strVideoDevConf = "AP"
			...
			' Parse advanced WMV settings

			case "-v_compopt"
				g_intVideoCompOpt = ReadInteger( args, i, i+1 )
				if g_intVideoCompOpt < 0 or g_intVideoCompOpt > 2 then
					OutputInfo "Invalid range for parameter " & args(i)
					ParseParameters = false
					exit function
				end if
			...
			case "-v_denoise"
				g_intVideoDenoise = ReadInteger( args, i, i+1 )
				if g_intVideoDenoise < 0 or g_intVideoDenoise > 5 then
					OutputInfo "Invalid range for parameter " & args(i)
					ParseParameters = false
					exit function
				end if
			...
			case "-v_dquantoption"
				g_intVideoDquantOption = ReadInteger( args, i, i+1 )
				if g_intVideoDquantOption < 0 or g_intVideoDquantOption > 3 then
					OutputInfo "Invalid range for parameter " & args(i)
					ParseParameters = false
					exit function
				end if
			...
			case "-v_dquantstrength" 
				g_intVideoDquantStrength = ReadInteger( args, i, i+1 ) 
				if g_intVideoDquantStrength < 0 or g_intVideoDquantStrength > 4 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
			...
			case "-v_bdeltaqp" 
				g_intVideoBDeltaQP = ReadInteger( args, i, i+1 ) 
				if g_intVideoBDeltaQP < 0 or g_intVideoBDeltaQP > 31 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false
					exit function 
				end if 
			...
			case "-v_loopfilter" 
				g_intVideoLoopfilter = ReadInteger( args, i, i+1 ) 
				if g_intVideoLoopfilter < 0 or g_intVideoLoopfilter > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_median" 
				g_intVideoMedian = ReadInteger( args, i, i+1 ) 
				if g_intVideoMedian < 0 or g_intVideoMedian > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_edgeremoval" 
				g_intVideoEdgeRemoval = ReadInteger( args, i, i+1 ) 
				if g_intVideoEdgeRemoval < 0 or g_intVideoEdgeRemoval > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_threadaffinity" 
				g_intVideoThreadAffinity = ReadInteger( args, i, i+1 ) 
				if g_intVideoThreadAffinity < 0 or g_intVideoThreadAffinity > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_numthreads" 
				g_intVideoNumThreads = ReadInteger( args, i, i+1 ) 
				if g_intVideoNumThreads <> 1 and g_intVideoNumThreads <> 2 and g_intVideoNumThreads <> 4 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_rangeredux" 
				intVideoRangeReduxYR = ReadInteger( args, i, i+1 )
				intVideoRangeReduxYE = ReadInteger( args, i, i+2 )
				intVideoRangeReduxCR = ReadInteger( args, i, i+3 )
				intVideoRangeReduxCE = ReadInteger( args, i, i+4 )
				if intVideoRangeReduxYR < 0 or intVideoRangeReduxYR > 8 or intVideoRangeReduxYE < 0 or intVideoRangeReduxYE > 8 or intVideoRangeReduxCR < 0 or intVideoRangeReduxCR > 8 or intVideoRangeReduxCE < 0 or intVideoRangeReduxCE > 8 then
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function
				end if
				
				'Convert into hexadecimal 0x0Y0y0C0c format
				g_intVideoRangeRedux = intVideoRangeReduxYR * 16777216 + intVideoRangeReduxYE * 65536 + intVideoRangeReduxCR * 256 + intVideoRangeReduxCE
 
				i = i + 4 
 
			case "-v_overlap" 
				g_intVideoOverlap = ReadInteger( args, i, i+1 ) 
				if g_intVideoOverlap < 0 or g_intVideoOverlap > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_scaling" 
				g_intVideoScaling = ReadInteger( args, i, i+1 ) 
				if g_intVideoScaling < 0 or g_intVideoScaling > 2 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_lookahead" 
				g_intVideoLookahead = ReadInteger( args, i, i+1 ) 
				if g_intVideoLookahead < 0 or g_intVideoLookahead > 16 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
 			...
 			case "-v_lrc" 
				g_intVideoLookaheadRC = ReadInteger( args, i, i+1 ) 
				if g_intVideoLookaheadRC < 0 or g_intVideoLookaheadRC > 150 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_mbmodecost" 
				g_intVideoMBModeCost = ReadInteger( args, i, i+1 ) 
				if g_intVideoMBModeCost < 0 or g_intVideoMBModeCost > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_mmatch" 
				g_intVideoMMatch = ReadInteger( args, i, i+1 ) 
				if g_intVideoMMatch < 0 or g_intVideoMMatch > 2 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if
				g_intVideoMMatch = g_intVideoMMatch - 1
 
				i = i + 1 
 
			case "-v_mslevel" 
				g_intVideoMSLevel = ReadInteger( args, i, i+1 ) 
				if g_intVideoMSLevel < 0 or g_intVideoMSLevel > 4 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
			...
			case "-v_msrange" 
				g_intVideoMSRange = ReadInteger( args, i, i+1 ) 
				if g_intVideoMSRange < 0 or g_intVideoMSRange > 4 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
				g_intVideoMSRange = g_intVideoMSRange - 1
 
				i = i + 1 
 
			case "-v_mvcoding" 
				g_intVideoMVCoding = ReadInteger( args, i, i+1 ) 
				if g_intVideoMVCoding < 0 or g_intVideoMVCoding > 3 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_mvcost" 
				g_intVideoMVCost = ReadInteger( args, i, i+1 ) 
				if g_intVideoMVCost < 0 or g_intVideoMVCost > 1 then 
				OutputInfo "Invalid range for parameter " & args(i) 
				ParseParameters = false 
				exit function 
				end if 
			...
			case "-v_bframedist" 
				g_intVideoBFrameDist = ReadInteger( args, i, i+1 ) 
				if g_intVideoBFrameDist < 0 or g_intVideoBFrameDist > 7 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if
			...
			case "-v_percopt" 
				g_intVideoPercOpt = ReadInteger( args, i, i+1 ) 
				if g_intVideoPercOpt < 0 or g_intVideoPercOpt > 23 then 
					OutputInfo "Invalid range for parameter " & args(i)
					ParseParameters = false 
					exit function 
				end if 
				' Disallow usage of old dquant which might create illegal bitstreams
				if g_intVideoPercOpt = 1 then
					g_intVideoPercOpt = 4
				end if 
				OutputInfo "Warning: -v_percopt option has been deprecated and replaced with -v_adz."
				OutputInfo "         See help listing for more details."
 
				i = i + 1 
				
			case "-v_adz" 
				if g_intVideoPercOpt = -128 then
					g_intVideoPercOpt = ReadInteger( args, i, i+1 ) 
					if g_intVideoPercOpt < 0 or g_intVideoPercOpt > 20 then 
						OutputInfo "Invalid range for parameter " & args(i) 
						ParseParameters = false 
						exit function 
					end if
					if g_intVideoPercOpt > 0 then
						g_intVideoPercOpt = g_intVideoPercOpt + 3
					end if
				end if
			...
			case "-v_type" 
				g_intVideoType = ReadInteger( args, i, i+1 ) 
				if g_intVideoType < 0 or g_intVideoType > 4 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
			...
			case "-v_codedwidth" 
				g_intVideoCodedWidth = ReadInteger( args, i, i+1 ) 
				if g_intVideoCodedWidth < 32 or g_intVideoCodedWidth > 8192 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
			...
			case "-v_codedheight" 
				g_intVideoCodedHeight = ReadInteger( args, i, i+1 ) 
				if g_intVideoCodedHeight < 32 or g_intVideoCodedHeight > 8192 then 
					OutputInfo "Invalid range for parameter " & args(i) 
					ParseParameters = false 
					exit function 
				end if 
			...
			case "-v_preset" 
				' Get video preset
				g_strVideoPreset = ReadString( args, i, i+1 )
				g_strVideoPreset = LCase( g_strVideoPreset )
				
				if g_strVideoPreset = "" or ( g_strVideoPreset <> "fast" and g_strVideoPreset <> "good" and g_strVideoPreset <> "better" and g_strVideoPreset <> "best" and g_strVideoPreset <> "insane" ) then
					OutputInfo "Invalid preset name used. Available presets are: fast, good, better, best, insane."
					ParseParameters = false
					exit function
				end if		
				
				select case g_strVideoPreset
				
					case "fast" ' Complexity = 2 / Lookahead = 16 / Loopfilter = On / Overlap = On / B frames = 1
						if g_intVideoPerformance = -1 then g_intVideoPerformance = 40
						if g_intVideoLookahead = -128 then g_intVideoLookahead = 16
						if g_intVideoLoopfilter = -128 then g_intVideoLoopfilter = 1
						if g_intVideoOverlap = -128 then g_intVideoOverlap = 1
						if g_intVideoBFrameDist = -128 then g_intVideoBFrameDist = 1
												
					case "good" ' Complexity = 3 / Lookahead = 16 / Loopfilter = On / B frames = 1 
						if g_intVideoPerformance = -1 then g_intVideoPerformance = 60
						if g_intVideoLookahead = -128 then g_intVideoLookahead = 16
						if g_intVideoLoopfilter = -128 then g_intVideoLoopfilter = 1
						if g_intVideoBFrameDist = -128 then g_intVideoBFrameDist = 1
												
					case "better" ' Complexity = 3 / Lookahead = 16 / Loopfilter = On / B frames = 1 / MS Level = Integer Chroma / MS Range = Auto 
						if g_intVideoPerformance = -1 then g_intVideoPerformance = 60
						if g_intVideoLookahead = -128 then g_intVideoLookahead = 16
						if g_intVideoLoopfilter = -128 then g_intVideoLoopfilter = 1
						if g_intVideoBFrameDist = -128 then g_intVideoBFrameDist = 1
					...
					case "best" ' Complexity = 5 / Lookahead = 16 / Loopfilter = On / B frames = 1 / MS Range = Auto
						if g_intVideoPerformance = -1 then g_intVideoPerformance = 100
						if g_intVideoLookahead = -128 then g_intVideoLookahead = 16
						if g_intVideoLoopfilter = -128 then g_intVideoLoopfilter = 1
						if g_intVideoBFrameDist = -128 then g_intVideoBFrameDist = 1
						if g_intVideoMSRange = -128 then g_intVideoMSRange = -1
						
					case "insane" 'Complexity = 4 / Lookahead = 16 / Loopfilter = On / B frames = 1 / MS Level = True Chroma / MS Range = Auto / Motion Match = Auto
						if g_intVideoPerformance = -1 then g_intVideoPerformance = 80
						if g_intVideoLookahead = -128 then g_intVideoLookahead = 16
						if g_intVideoLoopfilter = -128 then g_intVideoLoopfilter = 1
						if g_intVideoBFrameDist = -128 then g_intVideoBFrameDist = 1
						if g_intVideoMSLevel = -128 then g_intVideoMSLevel = 2
						if g_intVideoMSRange = -128 then g_intVideoMSRange = -1
			...
			case "-log"
				' Print all output to log file
				g_strLogFile = ReadString( args, i, i+1 )
				if g_strLogFile <> "" then
					on error resume next
					g_strLogFile = g_objFileSystem.GetAbsolutePathname(g_strLogFile)
					set g_objLogFile = g_objFileSystem.CreateTextFile( g_strLogFile, true)
					if IsNull(g_objLogFile) then
						OutputInfo "Invalid log file destination."
						ParseParameters = false
						exit function
					else
						g_blnLogOutput = true
			...
			case "-album"						
				' Get the album of the output file.
				g_strAlbum = ReadString( args, i, i+1 )
		
				i = i + 1

			case "-trackno"						
				' Get the track number of the output file.
				g_intTrackNo = ReadString( args, i, i+1 )
		
				i = i + 1

			case "-year"						
				' Get the year of the output file.
				g_strYear = ReadString( args, i, i+1 )
		
				i = i + 1

			case "-genre"						
				' Get the genre of the output file.
				g_strGenre = ReadString( args, i, i+1 )
	...
	if strSaveToCfgFile <> "" then
		...
		objTextFile.Write( strArgumentsQuoted )
		objTextFile.close
	end if
...
function ShowHelp()
	OutputInfo g_strProductName
	OutputInfo g_strProductCopyright
	OutputInfo ""
	OutputInfo ""
	...
	OutputInfo "[-log] <log file>"
	OutputInfo "    Writes all text output to a log file. Always overwrites."
	...
	OutputInfo "[-saveprofile] <profile file name>"
	...
	OutputInfo ""
	OutputInfo "[-a_input]"
	OutputInfo "    Sets a separate input file to be used as audio source when encoding"
	OutputInfo "    to a .wmv containing both video and audio streams."
	OutputInfo "    Must use -input to also specify the video source, otherwise -a_input"
	OutputInfo "    will be ignored. Audio source defined by -a_input will override any"
	OutputInfo "    audio stream present in the main -input source."
	OutputInfo "    Enclose file names that have spaces in quotations."
	OutputInfo "    For example: -a_input ""c:\my sample.wav"""
	OutputInfo "[-a_codecs]"
	OutputInfo "    Lists all available audio codecs."
	...
	OutputInfo "    WMASTD:  Windows Media Audio (default)."
	OutputInfo "    WMAPRO:  Windows Media Audio Professional."
	OutputInfo "    WMALSL:  Windows Media Audio Lossless."
	OutputInfo "    WMSP:    Windows Media Audio Voice."
	OutputInfo "    PCM:     No compression."
	OutputInfo ""
	OutputInfo "    Legacy indexes WMA9STD, WMA9PRO, WMA9LSL and WMSP9 will translate"
	OutputInfo "    to new version-agnostic indexes."
	OutputInfo "[-a_mode] <mode>"
	OutputInfo "    Audio encoding mode to be used."
	...
	OutputInfo "    Audio content mode for the Windows Media Audio Voice codec."
	...
	OutputInfo "    You must also specify the Windows Media Audio Voice codec."
	...
	OutputInfo ""
	OutputInfo "    If you use the Windows Media Audio Professional codec:" 
	...
	OutputInfo ""
	OutputInfo "    If you use the Windows Media Audio Lossless codec:"
	...
	OutputInfo ""
	OutputInfo "[-v_codecs]"
	OutputInfo "    Lists all available video codecs."
	OutputInfo "[-v_codec] <codec index>"
	OutputInfo "    Video codec to be used. Use -v_codecs to list available video codecs."
	OutputInfo "    Specify codec index:"
	OutputInfo "       WMV9:   Windows Media Video 9 Simple & Main Profiles (default)."
	OutputInfo "       WVC1:   Windows Media Video 9 Advanced Profile."
	OutputInfo "       WMS9:   Windows Media Video 9 Screen."
	OutputInfo "       WMV8:   Windows Media Video V8."
	OutputInfo "       WMV7:   Windows Media Video V7."
	OutputInfo "       UNCOMP: Full frames (uncompressed)."
	OutputInfo "[-v_mode] <mode>"
	OutputInfo "    Video encoding mode to be used."
	OutputInfo "    0: 1-pass CBR (default)."
	OutputInfo "    1: 2-pass CBR."
	OutputInfo "    2: Quality-based VBR."
	OutputInfo "    3: Bit rate-based VBR (two-pass)."
	...
	OutputInfo "    Video frame rate in floating point to be encoded. Default: Source video"
	OutputInfo "    frame rate, or 30 fps."
	...
	OutputInfo "    0:  None (for progressive video input). Default."
	OutputInfo "    1, 2, 3, 4:"
	OutputInfo "        Deinterlace but preserve source size and framerate."
	OutputInfo "        For example, 640x480x30 interlace to 640x480x30 progressive."
	OutputInfo "        The encoder uses a simple field blending deinterlace method which"
	OutputInfo "        produces good quality in still scenes but might result in ghosting"
	OutputInfo "        or blurring in high motion scenes."
	...
	OutputInfo "        pattern."
	OutputInfo "    16: Interlaced encoding - Auto detect field order."
	...
	OutputInfo "    Higher values might drop frames in order to maintain image quality."
	...
	OutputInfo "    In 2-pass modes the quality parameter is ignored."
	OutputInfo "[-v_qp] <quantizer parameter>"
	OutputInfo "    An alternative method of specifying image quality in quality-based VBR"
	OutputInfo "    mode. Because quality-based VBR uses a fixed QP for all encoded frames,"
	OutputInfo "    this method allows direct mapping to QP levels. Valid range is 1.0 to"
	OutputInfo "    31.0 in 0.5 intervals. Default: 2.5"
	OutputInfo "    If v_quality is also specified it will override v_qp."
	...
	OutputInfo "[-v_performance] <performance level>"
	OutputInfo "    Sets the encoder performance/quality level. Possible values: 0, 20, 40,"
	OutputInfo "    60, 80, and 100, where 0 is fastest and 100 is highest quality. If you"
	OutputInfo "    do not specify a value, codec defaults are used."
	OutputInfo "    Default: 60 (WMV9 and WVC1)."
	OutputInfo "[-v_complexity] <complexity level>"
	OutputInfo "    Same as v_performance but expressed on a scale from 0 to 5."
	OutputInfo "    If v_performance is also specified it will override v_complexity."
	OutputInfo "[-v_profile] <conformance profile>"
	OutputInfo "    Sets the VC-1 conformance profile for the encoded video. Video profiles"
	OutputInfo "    are used to indicate the decoding complexity of the video, as well as to"
	OutputInfo "    indicate which features a decoder needs to have implemented in order to"
	OutputInfo "    correctly decode the video. In order to stay conformant to the requested"
	OutputInfo "    profile, the encoder might ignore certain advanced WMV settings."
	OutputInfo "    Possible values for WMV9 codec are SP (Simple) and MP (Main). WVC1 codec"
	OutputInfo "    only allows AP (Advanced) value."
	OutputInfo "    If you do not add the -v_profile option to a command, the correct profile"
	OutputInfo "    level is selected automatically during encoding."
	OutputInfo "[-pixelratio] <x y>"
	OutputInfo "    Specifies the video pixel aspect ratio. Common pixel aspect ratios:"
	OutputInfo "    DV NTSC fullscreen    10:11"
	OutputInfo "    DV NTSC widescreen    40:33"
	OutputInfo "    DV PAL fullscreen     12:11"
	OutputInfo "    DV PAL widescreen     16:11"
	OutputInfo "[-pixelformat] <format>"
	OutputInfo "    Specifies the source video pixel format. Possible values:"
	OutputInfo "    I420"
	OutputInfo "    IYUV"
	OutputInfo "    YV12"
	OutputInfo "    UYVY"
	...
	OutputInfo "    RGB565"
	OutputInfo "    RGB8"
	OutputInfo "    By default the encoder will try to autodetect the source pixel format."
	OutputInfo "[-videoonly]"
	OutputInfo "    Encodes video stream only."
	OutputInfo ""
	OutputInfo ""
	OutputInfo "Usage for advanced Windows Media Video settings."
	OutputInfo "------------------------------------------------"
	OutputInfo "    Valid only with -v_codec set to WVC1 or WMV9."
	OutputInfo ""
	OutputInfo "[-v_compopt] <preset>"
	OutputInfo "    Compression optimization preset."
	OutputInfo "    0: Off (default)."
	OutputInfo "    1: Best visual quality. Automatically sets:"
	OutputInfo "       -v_bframedist 1"
	OutputInfo "       -v_mslevel 1 (Integer Chroma)"
	OutputInfo "       -v_msrange 0 (Adaptive)"
	OutputInfo "       -v_mvcost 1 (Adaptive)"
	OutputInfo "       -v_mmatch 0 (Adaptive)"
	OutputInfo "       -v_dquantoption 2 (I & P frames)"
	OutputInfo "       -v_loopfilter 1 (On)"
	OutputInfo "       -v_complexity 3"
	OutputInfo "    Setting additional parameters will override any values set by the preset."
	OutputInfo "[-v_dquantoption] <option>"
	OutputInfo "    Specifies the type of frames to apply differential quantization to."
	OutputInfo "    Dquant allows macroblocks containing smooth or dark areas to be encoded at"
	OutputInfo "    a quantizer level lower than the rest of the image."
	OutputInfo "    0: Off (default)."
	OutputInfo "    1: Apply to I frame only."
	OutputInfo "    2: Apply to I and P frames."
	OutputInfo "    3: Apply to I, P and B frames."
	OutputInfo "[-v_dquantstrength] <strength>"
	OutputInfo "    Specifies the method and strength of dquant application. Only valid"
	OutputInfo "    when used with -v_dquantoption parameter set greater than 0."
	OutputInfo "    0: Regular dquant method (default)."
	OutputInfo "    1: Fixed QP dquant method for smooth areas (QP 2)."
	OutputInfo "    2: Fixed QP dquant method for very smooth areas (QP 1)."
	OutputInfo "    3: Fixed QP dquant method for dark areas (QP 1)."
	OutputInfo "    4: Regular dquant method for dark areas."
	OutputInfo "[-v_adz] <strength>"
	OutputInfo "    Controls adaptive dead-zone, a quantization method which attempts to"
	OutputInfo "    balance the bit distribution within a picture between smooth areas with"
	OutputInfo "    fine details and textured areas with coarse details. Increasing the"
	OutputInfo "    strength controls how aggressively areas are mapped to larger dead-zones"
	OutputInfo "    where image detail is irretrievably lost."
	OutputInfo "    Valid range is 0 to 15, where 0 (default) disables the ADZ option."
	OutputInfo "    Using high values can result in loss of too much image detail."
	OutputInfo "[-v_loopfilter] <setting>"
	OutputInfo "    Controls the in-loop deblocking filter. This filter reduces blocking"
	OutputInfo "    artifacts at encoding time so that future P and B frames don't carry"
	OutputInfo "    forward the blocking artifacts. If not specified, loop filter is"
	OutputInfo "    enabled by default at resolutions below standard definition (SD)."
	OutputInfo "    0 to disable, 1 to enable."
	OutputInfo "[-v_overlap] <setting>"
	OutputInfo "    Controls overlap smoothing. This filter smooths borders of adjacent"
	OutputInfo "    macroblocks in an encoded video. If not specified, overlap smoothing"
	OutputInfo "    is enabled by default at resolutions below standard definition (SD)."
	OutputInfo "    0 to disable, 1 to enable."
	OutputInfo "[-v_denoise] <setting>"
	OutputInfo "    Controls the noise removal filter. Valid range is 0 to 5, where 0"
	OutputInfo "    (default) disables denoising and 5 uses strongest denoising."
	OutputInfo "[-v_median] <setting>"
	OutputInfo "    Controls the median filter. This filter tells the codec to ignore noise"
	OutputInfo "    and grain when performing motion estimation which can improve video"
	OutputInfo "    quality in very noisy video but can adversely affect clean video."
	OutputInfo "    0 to disable (default), 1 to enable."
	OutputInfo "[-v_scaling] <setting>"
	OutputInfo "    Specifies whether to scale the luma range before encoding in order to"
	OutputInfo "    improve efficiency. Scaled luma range is not expanded when decoded."
	OutputInfo "    0: Off (default)."
	OutputInfo "    1: Conservative. Compresses Y range to 26..229."
	OutputInfo "    2: Aggressive. Compresses Y range to 31..224."
	OutputInfo "[-v_edgeremoval] <setting>"
	OutputInfo "    Specifies whether encoder should attempt to detect and remove noisy"
	OutputInfo "    frame edges, such as VBI data often found in broadcast television."
	OutputInfo "    0 to disable (default), 1 to enable."
	OutputInfo "[-v_mslevel] <level>"
	OutputInfo "    Specifies the level of color information used in motion search."
	OutputInfo "    0: Luma only (default)."
	OutputInfo "    1: Luma with nearest integer chroma."
	OutputInfo "    2: Luma with true chroma."
	'OutputInfo "    3: Macroblock-adaptive with nearest integer chroma."
	'OutputInfo "    4: Macroblock-adaptive with true chroma."
	OutputInfo "[-v_msrange] <range>"
	OutputInfo "    Controls the search range used in motion search."
	OutputInfo "    0: Frame-adaptive."
	OutputInfo "    1: +63.75/-64.0 H, +31.75/-32.0 V (default)."
	OutputInfo "    2: +127.75/-128.0 H, +63.75/-64.0 V."
	OutputInfo "    3: +511.75/-512.0 H, +127.75/-128.0 V."
	OutputInfo "    4: +1023.75/-1024.0 H, +255.75/-256.0 V."
	OutputInfo "[-v_mmatch] <method>"
	OutputInfo "    Specifies the method used to measure and identify motion."
	OutputInfo "    0: Macroblock-adaptive."
	OutputInfo "    1: SAD (default)."
	OutputInfo "    2: Hadamard."
	OutputInfo "[-v_mbmodecost] <method>"
	OutputInfo "    Specifies the cost method used to decide the macroblock mode."
	OutputInfo "    0: SAD/Hadamard (default when v_performance < 60)."
	OutputInfo "    1: Rate Distortion (default when v_performance > 40)."
	OutputInfo "[-v_mvcoding] <method>"
	OutputInfo "    Specifies the method used to code delta motion vectors in interlaced"
	OutputInfo "    images. Wider motion vector distributions are more efficient at storing"
	OutputInfo "    random motion within a frame."
	OutputInfo "    0: Default delta motion vector distribution size."
	OutputInfo "    1: Double horizontal delta motion vector distribution size."
	OutputInfo "    2: Double vertical delta motion vector distribution size."
	OutputInfo "    3: Double delta motion vector distribution size in both directions."
	OutputInfo "[-v_mvcost] <method>"
	OutputInfo "    Specifies the motion vector cost method."
	OutputInfo "    0: Fixed motion vector cost."
	OutputInfo "    1: Adaptive motion vector cost (default)."
	OutputInfo "[-v_bframedist] <frames>"
	OutputInfo "    Specifies the number of bidirectional predictive (B) frames that the"
	OutputInfo "    encoder will use between other types of frames."
	OutputInfo "    If not specified, the encoder will only use I and P type frames."
	OutputInfo "    Valid range is 0 to 7, where 0 indicates that B-frames will not be used."
	OutputInfo "[-v_bdeltaqp] <delta>"
	OutputInfo "    Sets the fixed increase between the anchor frame's picture quantizer level"
	OutputInfo "    and the B frame's picture quantizer level. Valid range is 0 to 31. If not"
	OutputInfo "    specified, the encoder will use variable deltas."
	OutputInfo "[-v_lookahead] <frames>"
	OutputInfo "    Sets the number of frames to evaluate ahead for ratecontrol optimization."
	OutputInfo "    Enabling lookahead might automatically insert B frames into the video in"
	OutputInfo "    place of flash frames. To completely disable B frame insertion, also set"
	OutputInfo "    -v_bframedist to 0. Lookahead is ignored in 2-pass ratecontrol modes."
	OutputInfo "    Valid range is 0 to 16, where 0 disables the lookahead option."
	OutputInfo "[-v_rangeredux] <Y reduction> <Y expansion> <C reduction> <C expansion>"
	OutputInfo "    Forces range reduction for the luma (Y) and chroma (C) channels. Reduced"
	OutputInfo "    range is then expanded on decode by the set expansion factor."
	OutputInfo "    Reducing the color/luma range reduces the encoded size of video frames,"
	OutputInfo "    but can introduce banding artifacts in the decoded frames."
	OutputInfo "    Valid range for each reduction/expansion factor is 0 to 8."
	OutputInfo "[-v_codedwidth] <width>"
	OutputInfo "    Encodes video at a width different than the decoded display width."
	OutputInfo "    Valid range is 32 to 8192. Values will be rounded up internally to the"
	OutputInfo "    nearest multiple of 32."
	OutputInfo "[-v_codedheight] <height>"
	OutputInfo "    Encodes video at a height different than the decoded display height."
	OutputInfo "    Valid range is 32 to 8192. Values will be rounded up internally to the"
	OutputInfo "    nearest multiple of 32."
	OutputInfo "[-v_numthreads] <threads>"
	OutputInfo "    Forces the encoder to use the specified number of threads for encoding."
	OutputInfo "    Valid values are 1, 2 and 4. If not specified, encoder will match the"
	OutputInfo "    number of logical processors reported by the operating system."
	OutputInfo "[-v_type] <type>"
	OutputInfo "    Specifies progressive, field or frame picture encoding method."
	OutputInfo "    For interlaced encoding you must also set -v_preproc to 16, 17 or 18."
	OutputInfo "    0: Progressive encoding (default when v_preproc < 16)."
	OutputInfo "    1: Interlaced frame encoding."
	OutputInfo "    2: Interlaced field encoding (default when v_preproc > 15)."
	OutputInfo "    3: Auto Interlaced frame/field switching."
	OutputInfo "    4: Auto Progressive/Interlaced frame/Interlaced field switching."
	OutputInfo "    The auto options are designed to pick the most efficient coding method"
	OutputInfo "    for every frame but do not affect the output frame rate or frame pattern."
	OutputInfo "[-v_preset] <preset>"
	OutputInfo "    Selects a set of predefined settings that help improve speed or quality."
	OutputInfo "    Available presets are:"
	OutputInfo "     fast: Up to 1.5x faster than default with comparable quality."
	OutputInfo "           -v_complexity 2"
	OutputInfo "           -v_bframedist 1"
	OutputInfo "           -v_lookahead 16"
	OutputInfo "           -v_loopfilter 1"
	OutputInfo "           -v_overlap 1"
	OutputInfo "     good: Up to 1.5x slower than default."
	OutputInfo "           -v_complexity 3"
	OutputInfo "           -v_bframedist 1"
	OutputInfo "           -v_lookahead 16"
	OutputInfo "           -v_loopfilter 1"
	OutputInfo "     better: Up to 2.5x slower than default."
	OutputInfo "           -v_complexity 3"
	OutputInfo "           -v_bframedist 1"
	OutputInfo "           -v_lookahead 16"
	OutputInfo "           -v_loopfilter 1"
	OutputInfo "           -v_mslevel 1"
	OutputInfo "           -v_msrange 0"
	OutputInfo "     best: Up to 4.5x slower than default."
	OutputInfo "           -v_complexity 5"
	OutputInfo "           -v_bframedist 1"
	OutputInfo "           -v_lookahead 16"
	OutputInfo "           -v_loopfilter 1"
	OutputInfo "           -v_msrange 0"
	OutputInfo "     insane: The slowest and highest quality preset."
	OutputInfo "           -v_complexity 4"
	OutputInfo "           -v_bframedist 1"
	OutputInfo "           -v_lookahead 16"
	OutputInfo "           -v_loopfilter 1"
	OutputInfo "           -v_mslevel 2"
	OutputInfo "           -v_msrange 0"
	OutputInfo "           -v_mmatch 0"
	OutputInfo "    Setting additional parameters will override any values set by the preset."
	OutputInfo "    Using video presets with -v_compopt 1 is not recommended."
	OutputInfo ""
	OutputInfo ""
	OutputInfo "Usage for metadata settings."
	OutputInfo "----------------------------"
	OutputInfo ""
	OutputInfo "[-title] <string>"
	OutputInfo "[-author] <string>"
	Outputinfo "[-album] <string>"
	Outputinfo "[-year] <string>"
	Outputinfo "[-trackno] <string>"
	Outputinfo "[-genre] <string>"
	OutputInfo "[-copyright] <string>"
	OutputInfo "[-description] <string>    "
	OutputInfo "[-rating] <string>"
        OutputInfo "NOTE: The maximum string length for each one is 255. It is recommended"
	OutputInfo "      that you enclose all string parameters in quotations.	"
...
function ListVideoCodecs()
	...
	for intVBRMode = 1 to 4
		...
		select case intVBRMode
			case WMENC_PVM_NONE
				OutputInfo "CBR Video Mode :"

			case WMENC_PVM_PEAK
				OutputInfo "Peak Bit Rate-Based VBR Video Mode :"

			case WMENC_PVM_UNCONSTRAINED
				OutputInfo "Quality-Based VBR Video Mode :"

			case WMENC_PVM_BITRATE_BASED
				OutputInfo "Bit Rate-Based VBR Video Mode :"
	...
	for intVBRMode = 1 to 4
		...
		for i=0 to objProfile.AudioCodecCount-1
			...
			if intVBRMode = WMENC_PVM_UNCONSTRAINED then
				...
				if strCodecId <> WMAPRO and strCodecId <> WMALSL then
			...
			For j=0 To objProfile.audioFormatCount(i) - 1
				...
				if intAudioSampleRate = 96000 then
					intAudioSampleRate = 96
				elseif intAudioSampleRate = 88200 then
					intAudioSampleRate = 88
				elseif intAudioSampleRate = 48000 then
					intAudioSampleRate = 48
...
function SetupProfile()
	...
	if g_strVideoCodec = WMV9 and g_strVideoDevConf = "AP" then
		OutputInfo "WMV9 video codec does not include Advanced Profile."
		OutputInfo "If you wish to use Advanced Profile, please specify -v_codec WVC1"
		SetupProfile = false
		exit function
	end if

	if g_strVideoCodec = WVC1 and (g_strVideoDevConf = "SP" or g_strVideoDevConf = "MP") then
		OutputInfo "WVC1 video codec does not include Simple and Main profiles."
		OutputInfo "If you wish to use Simple or Main Profile, please specify -v_codec WMV9"
		SetupProfile = false
		exit function
	end if


	'Set advanced WMV settings
	if g_strVideoCodec = WVC1 or g_strVideoCodec = WMV9 then
	
		' Check if a previous WMCmd encoding session was aborted and registry settings were left behind accidentally - "dirty" flag
		if not IsEncoderRegistryDirty() then 
			SaveEncoderRegistrySettings()
		end if
		ClearEncoderRegistrySettings()
		
		' Set dirty flag that ensures we can detect if registry settings were accidentally left behind due to an aborted encode session
		g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\WMCmdDirtyFlag", 1, "REG_DWORD"
				
		' Advanced Profile only
		if g_strVideoCodec = WVC1 then
		
			if g_intVideoCompOpt <> -128 then
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Compression Optimization Type", g_intVideoCompOpt, "REG_DWORD"
			end if
		
			if g_intVideoRangeRedux <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Range Reduction", g_intVideoRangeRedux, "REG_DWORD"
			end if
			
			if g_intVideoMVCoding <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Motion Vector Coding Method", g_intVideoMVCoding, "REG_DWORD"
			end if 
			
			if g_intVideoCodedWidth <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Encoding Width", g_intVideoCodedWidth, "REG_DWORD"
			end if 

			if g_intVideoCodedHeight <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Encoding Height", g_intVideoCodedHeight, "REG_DWORD"
			end if 

			if g_intVideoType <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\VideoType", g_intVideoType, "REG_DWORD"
			end if 
		end if
		
		' Advanced and Main Profiles only
		if not g_strVideoDevConf = "SP" then

			if g_intVideoDquantOption <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Dquant Option", g_intVideoDquantOption, "REG_DWORD"
			end if 
	 
			if g_intVideoDquantStrength <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Dquant Strength", g_intVideoDquantStrength, "REG_DWORD"
			end if 
			
			if g_intVideoBDeltaQP <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force B Frame Delta QP", g_intVideoBDeltaQP, "REG_DWORD"
			end if 
	 
			if g_intVideoLoopfilter <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force LoopFilter", g_intVideoLoopfilter, "REG_DWORD"
			end if 
			
			if g_intVideoMSRange <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Motion Search Range", g_intVideoMSRange, "REG_DWORD"
			end if 
			
			if g_intVideoBFrameDist <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\NumBFrames", g_intVideoBFrameDist, "REG_DWORD"
			end if 
	 
			if g_intVideoPercOpt <> -128 then 
				g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Perceptual Option", g_intVideoPercOpt, "REG_DWORD"
			end if
		end if

		
		' All profiles
		if g_intVideoDenoise <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\DenoiseOption", g_intVideoDenoise, "REG_DWORD"
		end if 
 
		if g_intVideoMedian <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Median", g_intVideoMedian, "REG_DWORD" 
		end if 
 
		if g_intVideoEdgeRemoval <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force NoiseEdgeRemoval", g_intVideoEdgeRemoval, "REG_DWORD"
		end if 
 
		if g_intVideoThreadAffinity <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Encoder Thread Affinity", g_intVideoThreadAffinity, "REG_DWORD"
		end if 
 
		if g_intVideoNumThreads <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force NumThreads", g_intVideoNumThreads, "REG_DWORD"
		end if 
 
		if g_intVideoOverlap <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Overlap", g_intVideoOverlap, "REG_DWORD"
		end if 
 
		if g_intVideoScaling <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Force Video Scaling", g_intVideoScaling, "REG_DWORD"
		end if 
 
		if g_intVideoLookahead <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Lookahead", g_intVideoLookahead, "REG_DWORD"
		end if 
		
		if g_intVideoLookaheadRC <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Lookahead Ratecontrol", g_intVideoLookaheadRC, "REG_DWORD"
		end if 
 
		if g_intVideoMBModeCost <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Macroblock Mode Cost Method", g_intVideoMBModeCost, "REG_DWORD"
		end if 
 
		if g_intVideoMMatch <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Motion Match Method", g_intVideoMMatch, "REG_DWORD"
		end if 
 
		if g_intVideoMSLevel <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Motion Search Level", g_intVideoMSLevel, "REG_DWORD"
		end if 
 
		if g_intVideoMVCost <> -128 then 
			g_objWshShell.RegWrite g_strVideoEncoderRegistryKey & "\Motion Vector Cost Method", g_intVideoMVCost, "REG_DWORD"
		end if 
...
function SetupDisplayInfo()
	...
	' Is album set?
	if g_strAlbum <> "" then
		objAttributes.Add "WM/AlbumTitle", CStr(g_strAlbum)		
	end if

	' Is genre set?
	if g_strGenre <> "" then
		objAttributes.Add "WM/Genre", CStr(g_strGenre)
	end if

	' Is track number set?
	if (g_intTrackNo <> "") and (IsNumeric(g_intTrackNo)) then
		objAttributes.Add "WM/TrackNumber", CStr(CInt(g_intTrackNo))
		objAttributes.Add "WM/Track", CStr(CInt(g_intTrackNo)-1)
	else g_intTrackNo = ""
	end if

	' Is year set?
	if (g_strYear <> "") and (IsNumeric(g_strYear)) then
		objAttributes.Add "WM/Year", CStr(g_strYear)
	else g_strYear = ""
	end if
...
function SetupPredefinedProfile(objAudience)
	...
	select case LCase( g_strProfile )
		...
		case "av1400":
			SetupAudioAudience objAudience, "WMASTD", 2, 44100, 128040, 16
		...
		case "a20_1":
			SetupAudioAudience objAudience, "WMASTD", 1, 22050, 20008, 16

		case "a20_2":
			SetupAudioAudience objAudience, "WMASTD", 2, 22050, 20008, 16

		case "a32":
			SetupAudioAudience objAudience, "WMASTD", 2, 32000, 32000, 16

		case "a48":
			SetupAudioAudience objAudience, "WMASTD", 2, 32000, 48024, 16

		case "a64":
			SetupAudioAudience objAudience, "WMASTD", 2, 44100, 64016, 16

		case "a96":
			SetupAudioAudience objAudience, "WMASTD", 2, 44100, 96024, 16

		case "a128":
			SetupAudioAudience objAudience, "WMASTD", 2, 44100, 128016, 16
...
' Given audio string(eg WMASTD) this returns the audio codec index
function GetAudioCodecIndex(strAudioCodec)
	dim intFourCC

	select case strAudioCodec
		case WMASTD
			intFourCC = WMASTD_FOURCC

		case WMAPRO
			intFourCC = WMAPRO_FOURCC

		case WMALSL
			intFourCC = WMALSL_FOURCC
...
function SetupCustomAudioProfile(objAudience)
	...
	if g_strAudioSetting <> "" then
		...
		if intAudioSampleRate = 96 then
			intAudioSampleRate = 96000
		elseif intAudioSampleRate = 88 then
			intAudioSampleRate = 88200
		elseif intAudioSampleRate = 48 then
			intAudioSampleRate = 48000
...
function SetupCustomVideoProfile(objAudience)
	...
	' For quality vbr mode use intSmoothness as VBR quality
	if g_objProfile.VBRMode(WMENC_VIDEO, 0) = WMENC_PVM_UNCONSTRAINED then
		if (g_intVideoQP <> -1) and (g_intVideoQuality = -1) then
			g_intVideoQuality = Int((31.01 - g_intVideoQP) / 0.3)
		end if
		if g_intVideoQuality <> -1 then
...
function LoadProfileDefaults()
	...
	if g_strAudioSetting = "" then
		if g_intAudioVBRMode = 2 and g_strAudioCodec = WMAPRO then
			g_strAudioSetting = "Q75_44_2_24"
		elseif g_intAudioVBRMode = 2 and g_strAudioCodec = WMALSL then
		...
		elseif g_strAudioCodec = WMAPRO then
...
function Transcode()
	...
	'Once the encoder has started it's safe to clear and restore registry settings
	if g_strVideoCodec = WVC1 or g_strVideoCodec = WMV9 then
		ClearEncoderRegistrySettings()
		RestoreEncoderRegistrySettings()
		g_objWshShell.RegDelete g_strVideoEncoderRegistryKey & "\WMCmdDirtyFlag"
	end if
...
function SetupInput()
	...
	if g_strInput <> "" then
		...
		if not g_objFileSystem.FileExists(strInput2) then
			err.Raise(53)
			OutputInfo "Input Error: 0x" & Hex(err.number) & " - " & err.description
			err.Clear
			SetupInput = false
			exit function
		end if
		...
		' Check for existance of alternate audio input
		if g_strAudioInput <> "" then
			dim strAudioInput2
			
			strAudioInput2 = g_objFileSystem.GetAbsolutePathName( g_strAudioInput )

			' Check for file existance
			if not g_objFileSystem.FileExists(strAudioInput2) then
				err.Raise(53)
				OutputInfo "Audio Input Error: 0x" & Hex(err.number) & " - " & err.description
				err.Clear
				SetupInput = false
				exit function
			end if
		...
		' If alternate audio source is defined, add video and audio sources separately
		elseif strAudioInput2 <> "" then
			set g_objVideoSource = g_objSourceGroup.AddSource( WMENC_VIDEO )
			g_objVideoSource.SetInput( strInput2 )
			set g_objAudioSource = g_objSourceGroup.AddSource( WMENC_AUDIO )
			g_objAudioSource.SetInput( strAudioInput2 )
		
		' Otherwise just automatically parse the single intput for streams
		else
...
function CheckForValidAudioBitRates( intBitRate )
	...
	select case intBitRate
		...
		case 1536000, 1411200,1280000,1024000,768000,705600,640000,440000,384000,383000,_
                     352800,320000,256000,255000,192000,191000,160000,128000,127000,96000,95000,_
                     80000,64000,63000,48000,40000,32000,24000,22000,20000,17000,16000,12000,10000,_
                     8000,6000,5000,4000,0
...
function EncodeFilesInFolder( strInputFolderName, strOutputFolderName )
	...
	for each objFile in objFileCollection
		...
		select case lcase(strFileType)
			case "avi", "wav", "mpg", "mpeg", "vob" , "wmv" , "wma", "mp3", "asf", "avs"
...
function CheckAndEnablePlaybackinWMP(ext)

	On Error Resume Next

	dim regAVS, intPermissions, intRuntime
			
	' Check if extension key already exists
	regAVS = g_objWshShell.RegRead("HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\")
	
	if err <> 0 then
		g_objWshShell.RegWrite "HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Permissions", 1, "REG_DWORD"
		g_objWshShell.RegWrite "HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Runtime", 7, "REG_DWORD"
	else
		intPermissions = -1
		intRuntime = -1
		
		intPermissions = g_objWshShell.RegRead("HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Permissions")
		intRuntime = g_objWshShell.RegRead("HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Runtime")
		
		if intPermissions < 1 then g_objWshShell.RegWrite "HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Permissions", 1, "REG_DWORD"
		if intRuntime < 7 then g_objWshShell.RegWrite "HKCU\Software\Microsoft\MediaPlayer\Player\Extensions\" & ext & "\Runtime", 7, "REG_DWORD"	
	end if

end function

function GetAlternateSourceDuration()

	On Error Resume Next

	dim intTimeOut
	dim intWMPMediaDuration
	dim objWMP
	dim strInput2
	dim blnWMPError

	objWMP = Null
	intWMPMediaDuration = -1
	blnWMPError = false
		
	strInput2 = g_objFileSystem.GetAbsolutePathName( g_strInput )

	OutputInfo "Warning: Encoder failed to retrieve duration attribute from source plug-in."
	WScript.Stdout.Write "         Trying alternate method now."

	' Initialize Windows Media Player ActiveX control
	set objWMP = WScript.CreateObject( "WMPlayer.OCX" )

	if not IsNull(objWMP) then
	
		' If source unknown, enable playback in WMP - override security dialog
		CheckAndEnablePlaybackinWMP(Right( LCase( g_strInput ), 4 ))

		' Mute player volume
		objWMP.settings.mute = True

		' Load source media
		objWMP.URL = strInput2

		intTimeOut = 0

		' Wait for the media to open or time out after 30 seconds
		do while (intTimeOut < 30) and not (objWMP.openState = 13)
		...
		' Proceed only if media was successfully opened
		if intTimeOut < 30 then

			if not IsNull(objWMP.currentMedia) then

				'  If media is not playing yet, wait another 3 seconds before retrieving duration
				if objWMP.playState <> 3 then WScript.Sleep(3000)
			
				intWMPMediaDuration = objWMP.currentMedia.duration
	
				if intWMPMediaDuration <> -1 then

					g_intAudioSourceDuration = intWMPMediaDuration * 1000
					g_intVideoSourceDuration = intWMPMediaDuration * 1000

					OutputInfo "Source duration retrieved: " & intWMPMediaDuration & " seconds."
				else
					blnWMPError = true
				end if
	...
	if blnWMPError then
		OutputInfo ""
		OutputInfo "Warning: Encoder failed to retrieve source duration again."
		OutputInfo "         Encode process might not continue or progress report may be inaccurate."
		g_intAudioSourceDuration = 0
		g_intVideoSourceDuration = 0
	end if
...
function ConvertSecondsToHHMMSS( intSeconds )

	' Takes time in seconds as input and outputs a string formatted as hh:mm:ss

	dim intHH, intMM, intSS
	dim strHH, strMM, strSS

	intSS = Int(intSeconds mod 60)
	intMM = Int((intSeconds\60) mod 60)
	intHH = Int(intSeconds\3600)
	...
	ConvertSecondsToHHMMSS = strHH & ":" & strMM & ":" & strSS
...
function ShowProgress()
	...
	dim intPasses, intLimit, intCurrentPosition, tPassStartTime, intTimeElapsed, intTimeLeft, intLastPosition

	OutputInfo g_strProductName
	OutputInfo g_strProductCopyright
	OutputInfo ""

	' Display metadata information passed to the encoder script

	if g_strTitle <> "" then
		OutputInfo "Title:     " & g_strTitle
	end if
	if (g_intTrackNo <> "") and (IsNumeric(g_intTrackNo)) then
		OutputInfo "Track No:  " & g_intTrackNo
	end if
	if g_strAuthor <> "" then
		OutputInfo "Artist:    " & g_strAuthor
	end if
	if g_strAlbum <> "" then
		OutputInfo "Album:     " & g_strAlbum
	end if
	if g_strYear <> "" then
		OutputInfo "Year:      " & g_strYear
	end if
	if g_strGenre <> "" then
		OutputInfo "Genre:     " & g_strGenre
	end if
	if g_strCopyright <> "" then
		OutputInfo "Copyright: " & g_strCopyright
	end if
	if g_strDescription <> "" then
		OutputInfo "Comment:   " & g_strDescription
	end if
	OutputInfo ""
	...
	' Check if source duration property is available - enable error handling
	On Error Resume Next

	if not IsNull(g_objAudioSource) then
		g_intAudioSourceDuration = g_objAudioSource.Duration
		if IsNull(g_objAudioSource.Duration) then
			g_blnDurationError = True
		end if
	end if

	if not IsNull(g_objVideoSource) then
		g_intVideoSourceDuration = g_objVideoSource.Duration
		if IsNull(g_objVideoSource.Duration) then
			g_blnDurationError = True
		end if
	end if

	if g_blnDurationError then
		' Try to retrieve the source duration through Windows Media Player OCX
		GetAlternateSourceDuration()
	end if

	On Error Goto 0


	' Compute file duration and compensate for mark in/out
	if not IsNull(g_objAudioSource) then
		if g_objAudioSource.MarkOut <> 0 then
			intADuration = g_objAudioSource.MarkOut - g_objAudioSource.MarkIn
		else
			intADuration = g_intAudioSourceDuration - g_objAudioSource.MarkIn
		end if
	end if
	if not IsNull(g_objVideoSource) then
		if g_objVideoSource.MarkOut <> 0 then
			...
			intVDuration = g_intVideoSourceDuration - g_objVideoSource.MarkIn
		end if
	end if
	...
	for j=0 to intPasses-1
		...
		do while g_objEncoder.RunState <> WMENC_ENCODER_STOPPED and g_intErrorCode = 0 and Int(intPercent) < intLimit

			if intDuration = 0 then
				WScript.stdout.Write "."
			else
				intPercent = g_objEncoder.Statistics.EncodingTime * 1000000 / intDuration
				intCurrentPosition = g_objEncoder.Statistics.EncodingTime*10

				' Check if the current position has lapped itself, indicating that 2nd pass has started
				if intPasses = 2 and j = 0 and intLastPosition > intCurrentPosition then
					exit Do
				end if

				' Time elapsed since start of current pass. Total encoding time is tracked separately
				intTimeElapsed = DateDiff("s", tPassStartTime, Now())
				' Add 0.001 to divisor to prevent division-by-zero errors
				intTimeLeft = ( (0.001 * intDuration / (intCurrentPosition+0.001)) * intTimeElapsed ) - intTimeElapsed

				if intTimeLeft < 0 then
					intTimeLeft = 0   ' Prevents ugly negative output
				end if

				WScript.stdout.Write vbCr

				if intPasses = 2 and j = 0 then
					WScript.stdout.Write "Read: "
				else
					WScript.stdout.Write "Encoded: "
				end if	

				WScript.stdout.Write Round(intCurrentPosition,1) & "s (" & Round(intPercent,1) & "%)    "
				WScript.stdout.Write "Elapsed: " & ConvertSecondsToHHMMSS(intTimeElapsed) & "    "
				WScript.stdout.Write "Left: " & ConvertSecondsToHHMMSS(intTimeLeft) & "   [" 
				WScript.stdout.Write Round(intCurrentPosition/(intTimeElapsed+0.001),2) & "x]     "
		...
		if g_intErrorCode <> 0 then 
			OutputInfo vbCrLf & "Encoder error: 0x" & Hex( g_intErrorCode )
			exit for
		end if
...
function TerminateEncoderProcess()
	...
	For Each objProcess in colProcessList
		if InStr(1, Replace(objProcess.CommandLine, Chr(34), ""), Trim(WScript.ScriptName & strArguments), 1 ) > 0 then
