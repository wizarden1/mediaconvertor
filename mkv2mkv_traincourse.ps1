...
$audio_languages= @($false,"eng") #@("Use manual set","track ID/default","track ID",...)
$select_audio_by=@("all",@("eng")) #select_audio_by:<language|trackid|all>,<list of languages|number of tracks> example1: @("all",@("jpn"))
$RecompressMethod="Decoder"     #"AviSynth"|"Decoder"
#$RecompressMethod="AviSynth"     #"AviSynth"|"Decoder"

#Video
$video_languages= @($false,"eng") #@("Use manual set","track ID/default","track ID",...)
...
$resize=@($false,1280,720,"lanczos","") #resize:enabled,width,height,method,", additional parametrs"
...
#  Check for Errors
  ...
  # Generate Video
    ...
    if ($debug) {
      "Run Command Cli: $mkvmerge_path --output ""$out\$($file.basename).mkv"" $videotrack_cli $audiotrack_cli $track_order"
	}
	Start-Process -Wait -NoNewWindow -FilePath $mkvmerge_path -ArgumentList "--output ""$out\$($file.basename).mkv"" $videotrack_cli $audiotrack_cli $track_order"
