...
#$DecompressSource="FFVideoSource"	#"FFVideoSource"|"DirectShowSource"|"Direct"
$DecompressSource="DirectShowSource"	#"FFVideoSource"|"DirectShowSource"|"Direct"
...
$x264_path = Join-Path $multimedia "x264\x264_64.exe"
$x265_path = Join-Path $multimedia "x265\x265_64-8bit[icc].exe"
$mkvmerge_path = Join-Path $multimedia "mkvtoolnix\mkvmerge.exe"
$mkvextract_path = Join-Path $multimedia "mkvtoolnix\mkvextract.exe"
$MediaInfoWrapper_path = Join-Path $root_path "MediaInfoWrapper.dll"
$oggdec_path = Join-Path $root_path "oggdec.exe"
$eac3to = Join-Path $multimedia "eac3to\eac3to.exe"
$faad_path = Join-Path $root_path "faad.exe"
$wavi = Join-Path $root_path "Wavi.exe"
$avs2yuv_path = Join-Path $multimedia "avs2yuv\avs2yuv64.exe"
...
# Audio Encoding
	Switch ($RecompressMethod)
	{
		"AviSynth"  {
                        ...
                        if ($debug){"AviSynth Command Line: $wavi ""$enctemp\videofile.avs"" ""$enctemp\$($medinfo.Audiotracks[0].GUID).pcm"""}
		...
		"Decoder"   {
						Foreach ($audiotrack in $medinfo.Audiotracks) {
                            if ($debug){"Decoder Command Line: $mkvextract_path tracks ""$enctemp\temp$extension.$extension"" $($audiotrack.ID-1):""$enctemp\$($audiotrack.GUID).$($audiotrack.Format)"""}
...
#  Extracting timecode
	Foreach ($videotrack in $medinfo.Videotracks) {
		...
		if ($debug){"mkvextract Command Line: $mkvextract_path timecodes_v2 ""$enctemp\temp$extension.$extension"" $($videotrack.ID-1):""$enctemp\$($videotrack.GUID).timecode"""}
...
#  Extracting Chapters
	if ($Copy_Chapters -and $medinfo.Chapters){
	        "Extracting chapters..."
		if ($debug){"mkvextract Command Line: $mkvextract_path chapters ""$enctemp\temp$extension.$extension"" -r ""$enctemp\chapters.xml"""}
...
#		Start-Process -Wait -NoNewWindow -FilePath $mkvmerge_path -ArgumentList "-o ""$enctemp\$($videotrack.GUID).$($videotrack.Format)"" --video-tracks $($videotrack.ID) --no-audio --no-global-tags --no-subtitles --no-track-tags --no-chapters --no-cues ""$enctemp\temp$extension.$extension"""
		if ($debug){"mkvextract Command Line: $mkvextract_path -o ""$enctemp\$($videotrack.GUID).$($videotrack.Format)"" --video-tracks $($videotrack.ID-1) --no-audio --no-global-tags --no-subtitles --no-track-tags --no-chapters --no-cues ""$enctemp\temp$extension.$extension"""}
