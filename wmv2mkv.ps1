...
$DecompressSource="DirectShowSource"	#"FFVideoSource"|"DirectShowSource"
$Copy_Chapters=$false

#Filters
$crop=@($false,20,20,20,20) #crop:enabled,left,top,right,bottom
$resize=@($false,640,480,"lanczos","") #resize:enabled,width,height,method,", additional parametrs"
...
$x264_path = Join-Path $meguitools "x264\x264.exe"
...
#	Start-Process -Wait -NoNewWindow -FilePath "fsutil" -ArgumentList "hardlink create ""$enctemp\temp$extension.$extension"" ""$($file.FullName)"""
        "Copying file $($file.FullName)"
        Copy-Item $($file.FullName) -destination "$enctemp\temp$extension.$extension"
...
#  Selecting Audio tracks
	...
	Foreach ($audiotrack in $medinfo.Audio) {
		...
		$audiotrack = Add-Member -memberType NoteProperty -name GUID -value "" -PassThru -inputObject $audiotrack
		$audiotrack.guid = [guid]::NewGuid().tostring()
...
#  Extracting timecode & Chapters
	Foreach ($videotrack in $medinfo.Video) {
		...
		$videotrack = Add-Member -memberType NoteProperty -name GUID -value "" -PassThru -inputObject $videotrack
		$videotrack.guid = [guid]::NewGuid().tostring()
...
#  Extracting Chapters
	if ($Copy_Chapters -and $($medinfo.MenuCount -gt 0)){
...
#		Start-Process -Wait -NoNewWindow -FilePath $mkvmerge_path -ArgumentList "-o ""$enctemp\$($videotrack.UniqueID).$($videotrack.Format)"" --video-tracks $($videotrack.ID) --no-audio --no-global-tags --no-subtitles --no-track-tags --no-chapters --no-cues ""$enctemp\temp$extension.$extension"""
		...
		{
			"DirectShowSource"   {"DirectShowSource(""temp$extension.$extension"")" | Out-File "$enctemp\$($videotrack.GUID).avs" -Append -Encoding Ascii}
			"FFVideoSource"   {"FFVideoSource(""temp$extension.$extension"")" | Out-File "$enctemp\$($videotrack.GUID).avs" -Append -Encoding Ascii}
			default	{throw "Unknown Recompress Method."}
		}

		Compress-Video $(Get-ChildItem "$enctemp\$($videotrack.GUID).avs") -quantanizer $quantanizer -preset $preset -tune $tune -crop $crop -resize $resize -pulldown $pulldown
...
#  Check for Errors
  ...
  # Generate Video
	...
	Foreach ($videotrack in $videotracks) {
	    ...
	    #$videotrack_cli += " --timecodes 0:""$enctemp\$($videotrack.GUID).timecode"""
	...
	if ($Copy_Chapters -and $($((Get-Item "$enctemp\chapters.xml").Length) -gt 0)){
		$chapters_cli = "--no-subtitles --no-chapters --chapters ""$enctemp\chapters.xml"""
	}
    if ($debug) {
...
if ($debug) {
	...
	"Chapters Track Cli: $chapters_cli"
